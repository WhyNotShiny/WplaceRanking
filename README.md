...
| Path | What it is |
|---|---|
| `index.html`, `styles.css`, `script.js` | The dashboard itself — a static, dependency-light web app |
...
wplace_leaderboard_builder.cs  →  region_leaderboard_YYYY-MM-DD.csv + manifest.json
        (runs ~2h, weekly,             (committed to Leaderboard_files/)
         via GitHub Actions)
                                              ↓
                                   script.js fetches manifest.json,
...
If you fork this repo, update the repo config constants near the top of `script.js`
